...
public class EntityParentLookupsPanel extends ModelibraPanel implements
	...
	public EntityParentLookupsPanel(ModelibraFrame contentFrame, boolean add,
			IEntities<?> entities, IEntity<?> entity,
			List<NeighborConfig> parentNeighborConfigList) {
	...
	protected void addParentLookups(boolean add, IEntities<?> entities,
			IEntity<?> entity, List<NeighborConfig> parentNeighborConfigList) {
		...
		for (NeighborConfig parentNeighborConfig : parentNeighborConfigList) {
			if (parentNeighborConfig.isUpdate()
				...
				add(new EntityParentLookupPanel(contentFrame, add, entities,
						entity, parentNeighborConfig));
			}
		}
	}
